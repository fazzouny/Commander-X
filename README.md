...
Create a GitHub issue template with diagnostics.
...
`/backup` writes a sanitized Commander configuration snapshot under `backups/`. It excludes `.env` values, tokens, Telegram user IDs, full local paths, logs, and command output. Use `/backup preview` to inspect the summary first, `/backup timeline` to see when safe config areas last changed, `/backup list` to see saved snapshots, `/backup check` to validate the latest snapshot, `/backup plan` to preview the manual restore steps, and `/backup import` to draft review-only config snippets without changing files. Use `/backup import save` to save that preview as an ignored review artifact, `/backup import list` to list saved drafts, `/backup import open [preview_id]` to reopen one, `/backup import compare` to compare the latest safe backup with current config names and counts, `/backup import impact` to explain the differences in operator language, and `/backup import prepare` to create an approval-gated review checkpoint that still does not write live config.
...
The Safe Backups card previews and saves the same sanitized config snapshots as `/backup`. It shows backup names and sizes only, plus restore guidance; backup contents and local paths are not exposed in the dashboard. Config Timeline shows when safe Commander config areas last changed and whether they have local Git changes, without inspecting `.env`, real allowlists, logs, paths, or secrets. Use `/backup check` or the dashboard Check Restore button to validate the latest backup before using it as a manual rebuild reference. Use `/backup plan` or Restore Plan for the dry-run steps, and `/backup import` or Import Preview for review-only config snippets; both confirm no files are changed. Save Import Draft stores the preview under ignored reports for later review and still does not touch live config. Saved Drafts and Open Latest Draft reopen those ignored review artifacts without showing local report paths. Compare Draft shows read-only name/count differences between the latest safe backup and current config. Explain Impact turns those differences into plain-English operator impact before approval, and the dashboard groups them into risk-labeled review cards with recommended actions. Prepare Apply Gate creates a pending approval for the import review, but even approval only records/saves a review artifact and never writes live Commander config.
