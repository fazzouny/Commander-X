...
The Project Completion card is an owner scorecard for non-technical review. It translates `/done` evidence into owner status, confidence, success-criteria progress, proof count, changed areas to review, attention items, and the safest next action. It uses friendly project names and sorts actionable projects ahead of unconfigured ones. Commander still must not call a project final unless the objective is set, all criteria have proof, verification exists, no blockers or approvals are pending, and local changes are settled.
